...
In this final step, we are going to add support to AWS Cognito in order to make several actions protected and available only to registered users.

Interaction to AWS Cognito is complex and we will not cover it in this workshop, so we facilitate this part solved. But you must know that Cognito is based on Oauth2 standard so you have to send authorization headers and you have to let API Gateway which functions are protected.

So we are going to do the following:

*  Create a Cognito Users Pool
*  Create a Cognito Authorizer
*  Create a new function `search-restaurants`
*  Protect this new function

## Cognito resources

As in the previous step, we are going to define new resources into two new files:

`resources/infraestructure/cognito.yml`
```yaml
Resources:
  CognitoUserPool:
    ...
    Properties:
      ...
      UserPoolName: ${ self:custom.serviceStage }
  ...
  CognitoUserPoolClient:
    ...
    Properties:
      ...
      ClientName: ${ self:custom.serviceStage }
      UserPoolId:
        Ref: CognitoUserPool
      ExplicitAuthFlows:
        - ADMIN_NO_SRP_AUTH
...
`resources/infraestructure/api-authorizer.yml`
```yaml
Resources:
...
We also have to add them to the resources section in the serverless yaml file:
...
To consolidate changes and create the Cognito resources in AWS, we have to deploy:
...
Now, we need to add a new function to our service definition that let us search restaurants according to user input.

The first thing we are going to do is create the `functions/search-restaurants.js` file:
...
Next, create the function definition in the serverless yaml file inside the functions section:
...
As you can see, we have added an `authorizer` subsection that contains a reference to the authorizer created in the previous section.

## Other changes

Again, we have to update either `get-index` function and our html file:
  ...
  <head>
    ...
    <script>
      ...
      function searchRestaurants() {
        ...
        if( theme == "" ) {
          window.location.replace( '/' + serviceStage );
        } else {
...
Finally, deploy your changes to AWS:
...
[<-- Previous step](../step-2/step-2.md)
